...
namespace CSR
{
    ...
    public class MCNETAPI
    {
        ...
        /// 平台类型
        /// </summary>
        public bool COMMERCIAL => mcommercial;

        // 注册事件回调管理，避免回收
        private readonly Dictionary<string, ArrayList> callbks = new Dictionary<string, ArrayList>();
        // 发送tick方法管理，避免回收
        private readonly Dictionary<object, object> ticks = new Dictionary<object, object>();

        private readonly IntPtr hLib;
        public MCNETAPI(string DLLPath, string ver, bool commercial)
        {
            mVersion = ver;
        ...
        {
            if (callbks.TryGetValue(k, out ArrayList al))
            {
                if (al != null)
                {
                    al.Add(cb);
        ...
        public bool cshook(int rva, IntPtr hook, out IntPtr org)
        {
            ...
            bool ret = ccshook != null && ccshook(rva, hook, out sorg);
            org = sorg;
            return ret;
        }
        /// <summary>
        ...
        {
            IntPtr sorg = org;
            bool ret = ccsunhook != null && ccsunhook(hook, out sorg);
            org = sorg;
            return ret;
        }
        /// <summary>
